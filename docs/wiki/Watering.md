...
> Mirrored from <https://thefarmerwasreplaced.wiki.gg/wiki/Watering>
> — CC BY-SA. Regenerate with `python3 tools/fetch_wiki.py`.

Plants grow faster when they are watered. The ground has a water level ranging from `0` to `1`. The function `get_water()` returns the water level of the ground it is over.
...
You can use water on your plants. One tank of water is automatically added to your inventory every 10 seconds. Upgrading `Unlocks.Watering` will double the amount of water you get every 10 seconds.
