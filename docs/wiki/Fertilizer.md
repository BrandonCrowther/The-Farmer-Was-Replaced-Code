...
> Mirrored from <https://thefarmerwasreplaced.wiki.gg/wiki/Fertilizer>
> — CC BY-SA. Regenerate with `python3 tools/fetch_wiki.py`.

At some point, waiting for the plants to grow is just not efficient enough anymore. Similar to water, you will automatically receive 1 fertilizer every 10 seconds, doubling with each upgrade.
...
When a plant is infected, half of its yield is turned into `Items.Weird_Substance` when it is harvested. Weird Substance can also be used on plants, which has the effect of toggling the infected status of the plant and all adjacent plants.
