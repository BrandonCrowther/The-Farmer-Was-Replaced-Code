...
> Mirrored from <https://thefarmerwasreplaced.wiki.gg/wiki/Costs>
...
`get_cost(Entities.Pumpkin)` returns `{Items.Carrot:1}`
...
`get_cost(Unlocks.Loops, 0)` returns `{Items.Hay:5}`
