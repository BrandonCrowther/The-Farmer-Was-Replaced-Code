...
> Mirrored from <https://thefarmerwasreplaced.wiki.gg/wiki/Senses>
...
`num_items(item)` returns how many of an item you have. For example `num_items(Items.Hay)` returns how much hay you have.
...
The `None` keyword is also unlocked now! `None` is a value that represents that there is no value. For example, a function that has no `return` statement will actually return `None`.
...
Be careful `num_unlocked(Unlocks.Carrots)` will return the number of times it was unlocked/upgraded. `num_unlocked(Items.Carrot)` will only return `0` or `1`. (Same for other plants)
