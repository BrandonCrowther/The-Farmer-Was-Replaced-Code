...
# exp-hay-053 -- reroll away from distance-3 draws too, not just Carrot
#
# The champion only rerolls to escape Entities.Carrot (needs till()).
# It never rerolls to escape an expensive *distance* -- a distance-3
# walk costs meaningfully more round-trip movement (2x641=1,282) than
# distance-1 (2x225=450). Reroll on (type==Carrot OR distance==3), same
# REROLL_LIMIT=2, to bias realized walks toward the cheap end.
#
...
def driver(x, y):
	...
	for i in range(CYCLES + 20):
		...
		while rerolls < REROLL_LIMIT and companion != None:
			...
			bad = ctype == Entities.Carrot or wdist(x, y, cx, cy) == 3
			if not bad:
				break
			harvest()
			instructions()
...
for c in range(CYCLES):
	...
	while rerolls < REROLL_LIMIT and companion2 != None:
		...
		bad2 = ctype2 == Entities.Carrot or wdist(ax, ay, cx2, cy2) == 3
		if not bad2:
			break
		harvest()
		instructions()
